...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
   ...
   <!-- hides TEI header -->
    ...
    <xsl:template match="//tei:div2">
        <xsl:variable name="part_num" select="position() - 1"/>
        <xsl:result-document
            href="{concat('/Users/amyya/OneDrive/Desktop/Capstone/','Part0',$part_num, '.html')}">
            <html>
                <head>
                    <title>Tacitus Histories</title>
                ...
                <body>
                  <nav>
                      <ul>
                         ...
                         <li><a href="Personography.html">Personography</a></li>
                         <li><a href="Placeography.html">Placeography</a></li>
  ...
  <!-- Turns anything in persName and placeName tag to links -->
...
<!-- Working on how to make a tooltip from Personography and Placeography data -->
 <!--  
    <xsl:template match="//tei:placeName">
        <xsl:variable name="matchPlaces">
            <xsl:value-of select="substring-after(@ref, '#')"/>
        </xsl:variable>
        <span class="tooltip">
              <xsl:value-of select="."/>
              <span class="tooltiptext" id="{@ref}"> 
              <xsl:apply-templates select="document('Placeography.xml')//tei:place[@xml:id =$matchPlaces]/tei:location/tei:geo"/>
              </span>
        </span>
    </xsl:template>
    -->
